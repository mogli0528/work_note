...
//string dataPath="C:/Users/Guoge/Desktop/test/im";    // The directory of the input images or the path of the input video
string dataPath="C:/Users/ggnbf/work/image&video/video/0927.avi";
string fileFormat="jpg";  // The file format of the input images
string savePath="C:/Users/ggnbf/work/results";    // The directory of the output images
//string savePath1="C:/Users/Guoge/study/code/c++/fast0317/fast0317/";  
//////////////////////// INPUT PARAMETERS END ////////////////////////
int histogramSize2D     = histogramSize1D * histogramSize1D;
int histogramSize3D     = histogramSize2D * histogramSize1D;
int logSize             = log((double)histogramSize1D)/log((double)2);
...
//read the image or vadio

void readFiles(vector<string>& imagePaths,vector<string>& imageNames,VideoCapture &cap,bool isimage)
{
    ...
    if(!isimage)
	{
        ...
        for (int i = 0; i < cap.get(CV_CAP_PROP_FRAME_COUNT); i=i+2) 
		{
           // string temp = to_string(i) + ".jpg";
			string temp = convertToString(i) + ".jpg";
                          ...
                          Mat &shapeProbability)
{
    ...
    //test
	vector<float> temdata;
	{
		
		for(int i=0;i<modelMean.rows;i++)
	    {
          for(int j=0;j<modelMean.cols;j++)
	     {
          temdata.push_back(modelMean.at<float>(i,j));
	     }
	   }
	}
    sqrt(12 * Vx, Xsize);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//system(("mkdir -p " + savePath).c_str());
	  //string sppath=savePath+"saliencyProbability/";
	  CString savepath(savePath.c_str());
      //CreateDirectory(str, NULL);
	  //str=+"saliencyProbability";
	  //CreateDirectory(str,NULL);
   //system(("mkdir -p " + savePath + "globalContrast/").c_str());
	  CreateDirectory(savepath+"globalContrast",NULL);
      //system(("mkdir -p " + savePath + "saliencyProbability/").c_str());
	  CreateDirectory(savepath+"saliencyProbability",NULL);
 //   system(("mkdir -p " + savePath + "saliencyMaps/").c_str());
	  CreateDirectory(savepath+"saliencyMaps",NULL);
 //   system(("mkdir -p " + savePath + "ellipseDetection/").c_str());
	  CreateDirectory(savepath+"ellipseDetection",NULL);
 //   system(("mkdir -p " + savePath + "rectangleDetection/").c_str());
	  CreateDirectory(savepath+"rectangleDetection",NULL);
 //   system(("mkdir -p " + savePath + "rectangleBoundingBoxes/").c_str());
	  CreateDirectory(savepath+"rectangleBoundingBoxes",NULL);
    ...
    float totalTime     = 0;
	 ...
	 for (int i = 0; i < squares.cols; i++)
        squaresPtr[i] = pow((double)i,2);



	// while(totalImages<imagePaths.size()) 
	 while(1)
	{
...
//////////////////////// SALIENCY COMPUTATION STARTS HERE ////////////////////////
            ...
            computeSaliencyMap(shapeProbability,
                               ...
                               SM,
                               saliency);
            //imwrite(savePath+imageNames[totalImages].substr(0,imageNames[totalImages].size()-4)+"_Ours"+ ".png", SM);
...
//////////////////////// SALIENCY COMPUTATION ENDS HERE ////////////////////////
            ...
            imwrite(savePath + "saliencyMaps/"  + imageNames[totalImages].substr(0,imageNames[totalImages].size()-4) +"_Ours"+ ".png", SM);
