...
/*
    ...
    \brief: the original guided filter for gray image. This function is used for image dehazing.
...
/*
    ...
    \brief: the original guided filter for rgb color image. This function is used for image dehazing.
        The video dehazing algorithm uses appoximated filter for fast refinement. 
    Parameter:
        nW - width of array
        nH - height of array
...
/*
    ...
    \brief: Transmission refinement based on guided fitlering, but we approximate the filtering 
...
/**
 *   \brief: Transmission refinement based on guided fitlering, 
 *           but we approximate the filtering using partial window. 
 *           In addtion, we analyze the guided filter using projection method.
 *           SSE (SIMD) is applied to accelerate.
 * 
 *           * Limitation - In this code, we do not consider the border line of 
 *             filtering, hence we will update the code.
 *            
 *           The user may regulate the window size with ratio that the image 
 *           size divides into the window size.
 * 
 *   \param: 
 *       nW - width of down-sampled image
 *       nH - height of down-sampled image 
...
/*
    ...
    \brief: It is a modified guided filter to reduce blur artifacts of original 
