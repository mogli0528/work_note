...
)
{
    ...
    int DenoiseLevel = DENOISE_LEVEL;
    ...
    cv::Mat color;
    int fps = 0;
    int resTop = 0, resArm = 0;

    while(mFrame_.data) {
        
        // denoise
        // cv::cvtColor(mFrame_, mFrame_, cv::COLOR_BGR2GRAY);
        IntIntegralImage ii(mFrame_);
        ii.calculate_sum_and_squaresum();

        if ( 1 == mFrame_.channels() )
            lee_filter_1_chs_ii(mFrame_, mFrame_, sigmma, ii);
        else if( 3 == mFrame_.channels() )
            lee_filter_3_chs_ii(mFrame_, mFrame_, sigmma, ii);

        // detect
        frames_++;
        start_ = boost::posix_time::microsec_clock::local_time();

        resTop = detectTopLine(topLineRoi, pt1TopLineDetect, pt2TopLineDetect);
        resArm = detectArmLine(armLineRoi, pt1ArmLineDetect, pt2ArmLineDetect);
        if(resTop < 0 && resArm < 0)
            return -1;
...
int TopLineDetector::detectTopLine(const cv::Rect &roi, cv::Point &pt1Detect, cv::Point &pt2Detect)
{
    ...
    // image_enhance(topLineAreaEnhanced_, topLineAreaEnhanced_, ENHANCE_LAPLACE, NULL);
    ...
    // cv::dilate(topLineAreaThreshold_, topLineAreaThreshold_, element3, cv::Point(-1, -1), 3);
    // cv::erode(topLineAreaThreshold_, topLineAreaThreshold_, element3, cv::Point(-1, -1));
    cv::erode(topLineAreaThreshold_, topLineAreaThreshold_, element3, cv::Point(-1, -1), 2);
    cv::GaussianBlur(topLineAreaThreshold_, topLineAreaThreshold_, cv::Size(ksize+2, ksize+2), 0, 0); 

    
    // 霍夫直线检测, 注意参数的选取   
    cv::Canny(topLineAreaThreshold_, topLineAreaEdges_, 50, 220, 5);

    // 霍夫直线变换函数 HoughLinesP() 的参数说明:   
    cv::HoughLinesP(topLineAreaEdges_, topLineAreaLines_, 9, CV_PI/180, 15, 20, 20);
    // 轮廓的显示需要反转黑白值  
...
int TopLineDetector::detectArmLine(const cv::Rect &roi, cv::Point &pt1Detect, cv::Point &pt2Detect)
{
    ...
    // image_enhance(armLineAreaEnhanced_, armLineAreaEnhanced_, ENHANCE_LAPLACE, NULL);
    ...
    cv::GaussianBlur(armLineAreaThreshold_, armLineAreaThreshold_, cv::Size(ksize+2, ksize+2), 0, 0); 

    
    // 霍夫直线检测, 注意参数的选取   
    cv::Canny(armLineAreaThreshold_, armLineAreaEdges_, 30, 200, 5);

    // 霍夫直线变换函数 HoughLinesP() 的参数说明:   
    cv::HoughLinesP(armLineAreaEdges_, armLineAreaLines_, 9, CV_PI/180, 15, 20, 20);
    // 轮廓的显示需要反转黑白值  
