...
class Blob {
 ...
 protected:
  ...
  shared_ptr<SyncedMemory> data_;    // data_ 用于前向传播, 指针类型是 shared_ptr 
  shared_ptr<SyncedMemory> diff_;   // diff_主要用来存储偏差，update data
  shared_ptr<SyncedMemory> shape_data_;
  vector<int> shape_;    // Blob 的形状
  int count_;     // 表示 Blob 中的元素个数，也就是 n*c*h*w 中的 n
