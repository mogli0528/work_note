...
|---|-------------|---|-------------|---|--------------|---|-------------|
| α |`$\alpha$`   | A | `$A$`       | β | `$\beta$`    | B | `$B$`       |
| γ |`$\gamma$`   | Γ | `$\Gamma$`  | δ | `$\delta$`   | Δ | `$\Delta$`  |
| ϵ |`$\epsilon$` | E | `$E$`       | ζ | `$\zeta$`    | Z | `$Z$`       |
| η |`$\eta$`     | H | `$H$`       | θ | `$\theta$`   | Θ | `$\Theta$`  |
| ι |`$\iota$`    | I | `$I$`       | κ | `$\kappa$`   | K | `$K$`       |
| λ |`$\lambda$`  | Λ | `$\Lambda$` | ν | `$\nu$`      | N | `$N$`       |
| μ |`$\mu$`      | M | `$M$`       | ξ | `$\xi$`      | Ξ | `$\Xi$`     |
| o |`$o$`        | O | `$O$`       | π | `$\pi$`      | Π | `$\Pi$`     |
| ρ |`$\rho$`     | P | `$P$`       | σ | `$\sigma$`   | Σ | `$\Sigma$`  |
| τ |`$\tau$`     | T | `$T$`       | υ | `$\upsilon$` | Υ | `$\Upsilon$`|
| ϕ |`$\phi$`     | Φ | `$\Phi$`    | χ | `$\chi$`     | X | `$X$`       |
...
以 a 为例, 如果字母 i 或 j 带有重音，字母i,y应该替换为 $\imath$, $\jmath$.  
